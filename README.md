...
3. 时间超过
当路由器收到一个目的IP地址不是自己的报文，就将报文的TTL生存时间减一，并将其转发出去；如果TTL为0，丢弃数据报，并向源点发送时间超过报文
4. 源点抑制
...
ICMP询问报文有两种
1. 回送请求和回答
ICMP回送请求和回答报文是主机或路由器向一个特定主机发出询问，收到此报文的主机必须给源主机或路由器发送ICMP回送回答报文。可用来测试目的主机是否可达和了解相关状态
2. 时间戳请求和回答
