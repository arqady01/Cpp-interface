...
---|---|---|---|---|---|---|---|---
vector|数组|支持随机访问，O(1)|O(n)|无序|可重复|1️⃣若尾插导致扩容，所有指向元素的迭代器、指针和引用都将失效2️⃣若插入（例如insert）无需内存重分配，那么插入点之前元素的迭代器依旧有效，而插入点及之后元素的迭代器均会失效3️⃣若插入操作导致内存重分配，那么所有迭代器、指针和引用都会失效|sad|asfas
