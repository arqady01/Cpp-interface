...
通常指堆内存的泄漏。使用malloc、new等函数从堆中分配到块内存，使用完后，程序必须调用free或delete释放该内存块，否则，这块内存就不能被再次使用，即内存泄漏
...
3. 避免对同一对象进行赋值操作，比如深浅拷贝时判断两个对象是否相同
...
动态绑定也称为晚期绑定，支持多态，必须使用虚函数实现，在**运行期**才确定具体调用哪个函数
