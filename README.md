...
#### 回环地址
127.0.0.0/8整个地址范围都被保留作为回环地址，用于在计算机网络中进行自我测试和诊断，标准回环地址是127.0.0.1

#### 子网掩码
子网掩码标志了两个IP是否同属于一个子网，指示IP地址中的哪些位代表网络部分，哪些位代表主机部分。

在计算时，假如IP是218.75.230.0，子网掩码是255.255.255.192，即255.255.255.11000000，可知从主机号中借2位作为子网号，划分的子网数为2^2=4（00、01、10、11），每个子网可分配的地址数量为2^(8-2)-2=62，详情如下：
...
| 218.75.230 | 01     | 000000 | 子网0的网络地址：218.75.230.64 |
| 218.75.230 | 01     | 000001 | 子网0可分配的最小地址：218.75.230.65 |
| 218.75.230 | 01     | 111110 | 子网0可分配的最大地址：218.75.230.126 |
| 218.75.230 | 01     | 111111 | 子网0的广播地址：218.75.230.127 |
| ......     | 10     | ...... | ......略                             |
...
+ 重置标志RST：重置由于主机崩溃或其他原因而出现错误的连接
+ 同步标志位SYN：在建立连接时同步序号
+ 终止标志位FIN：释放TCP连接
8. 窗口：以接收方的接受能力来控制发送发的发送能力，成为流量控制。发送窗口大小还取决于拥塞窗口大小，即min{接收窗口，拥塞窗口}
9. 紧急指针：指出数据载荷部分包含了多长的紧急数据

### TCP粘包

1. 要发送的数据大于TCP发送缓冲区大小
2. 待发送数据大于TCP最大报文长度，TCP在传输前将进行拆包
...
- 校验和：一般是CRC循环冗余校验，在IP数据报的首部检验和字段
- ARQ：自动请求重传ARQ协议，在数据链路层，通过确认和重传机制实现的可靠传输协议，类型有：停止等待SW、回退N帧GBN、选择重传SR
- 序列号：tcp基于字节流，对字节进行编号，序列号能防止：分组丢失，分组重复、分组失序
- 超时重传机制
